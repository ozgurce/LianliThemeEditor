...
public sealed class SupporterBridge : ISupporterBridge
{
    ...
    {
        var type = layer.Type ?? "";
        var useArchBarOuterBounds = UseArchBarOuterBounds(deviceModel);
        var layerX = useArchBarOuterBounds ? GetArchBarEngineCoordinate(layer, layer.X) : layer.X;
        var layerY = useArchBarOuterBounds ? GetArchBarEngineCoordinate(layer, layer.Y) : layer.Y;
        ...
        if (isGraph)
        {
            ...
            if (layer.CanWrite("diameter") && !string.IsNullOrWhiteSpace(layer.Diameter)) { args.Add("-LayerDiameter"); args.Add(useArchBarOuterBounds ? GetArchBarEngineDiameter(layer) : layer.Diameter); }
    ...
    public static void CleanupOrphanedBatchFiles()
    {
        try
        {
            var tempDir = Path.GetTempPath();
            var orphanFiles = Directory.GetFiles(tempDir, "lianli_layer_batch_*.json");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        args.AddRange(new[]
        {
            "-RenderGraphPreview",
            "-LayerIndex", layer.Index,
            "-LayerX", UseArchBarOuterBounds(deviceModel) ? GetArchBarEngineCoordinate(layer, layer.X) : layer.X,
            "-LayerY", UseArchBarOuterBounds(deviceModel) ? GetArchBarEngineCoordinate(layer, layer.Y) : layer.Y,
            "-PreviewValue", "100",
        ...
        AddIfPresent(args, "-LayerDiameter", UseArchBarOuterBounds(deviceModel) ? GetArchBarEngineDiameter(layer) : layer.Diameter);
    ...
    private static TemplateLoadResult ParseTemplateResult(string json)
    {
        ...
        var useArchBarOuterBounds = result.Width == 480 && result.Height == 480;
        if (root.TryGetProperty("Layers", out var layers) && layers.ValueKind == JsonValueKind.Array)
        {
            foreach (var layer in layers.EnumerateArray())
            {
                var row = new LayerRow
                {
                    ...
                    X = useArchBarOuterBounds ? GetArchBarEditorCoordinate(layer, "X") : GetValue(layer, "X"),
                    Y = useArchBarOuterBounds ? GetArchBarEditorCoordinate(layer, "Y") : GetValue(layer, "Y"),
                    ...
                    Diameter = useArchBarOuterBounds ? GetArchBarEditorDiameter(layer) : GetValue(layer, "Diameter"),
    ...
    private static string GetArchBarEditorCoordinate(JsonElement layer, string propertyName)
    {
        var value = GetValue(layer, propertyName);
        if (!IsArchBar(layer) ||
            !TryParseInvariant(value, out var coordinate) ||
            !TryParseInvariant(GetValue(layer, "Thickness"), out var thickness) ||
            thickness <= 0)
        {
            return value;
        }

        return FormatInvariant(coordinate - thickness / 2.0);
    }

    private static string GetArchBarEditorDiameter(JsonElement layer)
    {
        var value = GetValue(layer, "Diameter");
        if (!IsArchBar(layer) ||
            !TryParseInvariant(value, out var diameter) ||
            !TryParseInvariant(GetValue(layer, "Thickness"), out var thickness) ||
            thickness <= 0)
        {
            return value;
        ...
        return FormatInvariant(diameter + thickness);
    }

    private static string GetArchBarEngineCoordinate(LayerRow layer, string value)
    {
        if (!IsArchBar(layer) ||
            !TryParseInvariant(value, out var coordinate) ||
            !TryParseInvariant(layer.Thickness, out var thickness) ||
            thickness <= 0)
        {
            return value;
        }

        return FormatInvariant(Math.Round(coordinate + thickness / 2.0));
    }

    private static string GetArchBarEngineDiameter(LayerRow layer)
    {
        if (!IsArchBar(layer) ||
            !TryParseInvariant(layer.Diameter, out var diameter) ||
            !TryParseInvariant(layer.Thickness, out var thickness) ||
            thickness <= 0)
        {
            return layer.Diameter;
        }

        return FormatInvariant(Math.Max(1.0, Math.Round(diameter - thickness)));
    }

    private static bool IsArchBar(JsonElement layer) =>
        string.Equals(GetValue(layer, "Type"), "GraphArchBar", StringComparison.OrdinalIgnoreCase);

    private static bool IsArchBar(LayerRow layer) =>
        string.Equals(layer.Type, "GraphArchBar", StringComparison.OrdinalIgnoreCase);

    private static bool UseArchBarOuterBounds(string deviceModel) =>
