...
public sealed class SupporterBridge
{
    ...
    private async Task<string> RunSupporterAsync(IReadOnlyList<string> arguments, CancellationToken cancellationToken)
    {
        ...
        {
        if (string.Equals(
                Environment.GetEnvironmentVariable("LIANLI_THEME_SUPPORTER_TRACE"),
                "1",
                StringComparison.Ordinal))
        {
            try
            {
                ...
                sb.AppendLine($"[{DateTime.Now:yyyy-MM-dd HH:mm:ss}] Running template worker with args:");
            ...
            catch (Exception ex) { AppLogger.Error("Template worker call log could not be written.", ex); }
        }
