...
public static class FastProfileReader
{
    ...
    public static string GetActiveUniversalOrientation(string deviceModel)
    {
        try
        {
            var profileDir = Path.Combine(LConnectPaths.ProgramDataRoot, "profile");
            ...
            foreach (var file in Directory.GetFiles(profileDir).OrderByDescending(File.GetLastWriteTimeUtc))
            {
                try
                {
                    ...
                    if (IsUniversalDevice(deviceModel))
                    {
                        if (root.TryGetProperty("IsLandscape", out var isLandscapeProp) &&
                            (isLandscapeProp.ValueKind == JsonValueKind.True || isLandscapeProp.ValueKind == JsonValueKind.False))
                        {
                            return isLandscapeProp.GetBoolean() ? "landscape" : "portrait";
                        }
                    }
                }
                catch
    ...
    public static string GetActiveTemplateBackground(string deviceModel, string templateId)
    {
        ...
        {
            var profileDir = Path.Combine(LConnectPaths.ProgramDataRoot, "profile");
            var latestProfileWriteUtc = Directory.Exists(profileDir)
                ? Directory.GetFiles(profileDir)
                    .Select(path =>
                    {
                    ...
                    .Max()
                : DateTime.MinValue;
            var latestLogBackground = GetLatestAppliedTemplateBackgroundFromLogs(
                deviceModel,
                templateId,
                latestProfileWriteUtc);
    ...
    private static string ReadFileContent(string path)
    {
        try
        {
            ...
            {
                using var gzipMs = new MemoryStream(bytes);
    ...
    private static string GetLatestAppliedTemplateBackgroundFromLogs(
        string deviceModel,
        string templateId,
        DateTime minLogWriteUtc)
    {
        ...
        foreach (var file in Directory.GetFiles(logDir, "L-Connect-Service-*.log").OrderByDescending(File.GetLastWriteTimeUtc).Take(3))
        {
            try
            {
                if (minLogWriteUtc > DateTime.MinValue &&
                    File.GetLastWriteTimeUtc(file) < minLogWriteUtc.AddSeconds(-2))
                {
                    continue;
                }
            }
