...
- Fixed canvas pixel configuration updates when selecting/switching theme orientations.
- Fixed background image preview auto-rotation so standard images (.jpg, .png, .gif) and MP4 videos are not auto-rotated sideways.

## V 2.8.15

- Fixed image layer preview size: large source images (e.g. 1920×1080) are now fit to the canvas at ZoomRate=1 and scale proportionally, eliminating the "huge image at low zoom" issue.
- Fixed interactive resize of image layers in the preview canvas: resizing now correctly updates ZoomRate using the larger of the horizontal/vertical drag delta, so both axes work naturally.
- Fixed image preview rendering quality: applies `BitmapScalingMode.HighQuality` (Fant/bicubic), `SnapsToDevicePixels`, and `UseLayoutRounding` on all preview image controls for crisp rendering at any scale.
- Fixed gallery submission package format to always export as `.zip`.
- Fixed custom font packaging for non-standard Windows system fonts.
